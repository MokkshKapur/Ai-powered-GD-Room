...
async def gd_room(websocket: WebSocket):
    ...
    async def force_conclude():
        await websocket.send_json({
            "type": "control",
            "action": "force_stop"
        })
        closing = "Thank you everyone for your valuable insights. This concludes our group discussion."
        conversation.append(f"{MODERATOR['name']}: {closing}")
        await send_agent_message(MODERATOR, closing)
        print("📝 GD Log:")
        for line in conversation:
            print(line)

    try:
        intro = get_moderator_intro(topic)
        conversation.append(f"{MODERATOR['name']}: {intro}")
        if not await send_agent_message(MODERATOR, intro):
            return
        ...
        while True:
                ...
                if "text" in message:
                    try:
                        ...
                        if data.get("type") == "audio_chunk":
                            ...
                            if data.get("action") == "end_turn" and is_user_speaking:
                                await end_user_turn()
                                if last_audio:
                                    ...
                                    for agent in AGENTS:
                                        context = format_conversation_context(conversation, username)
                                        context += f"\n\nConsider the user's recent statement and respond to it directly:\n{username}: {final_text}"
                                        text = generate_agent_response(agent, topic, context)
                                        conversation.append(f"{agent['name']}: {text}")
                                        if not await send_agent_message(agent, text):
                                            return
                                    ...
                                    if round_count >= MAX_ROUNDS:
                                        await force_conclude()
                                        return
                                    await start_user_turn()

                            elif data.get("action") == "force_conclude":
                                if last_audio:
                                    print("⌛ Time up but user audio pending. Letting transcription finish...")
                                    final_text = transcribe_audio(last_audio)
                                    conversation.append(f"{username}: {final_text}")
                                await force_conclude()
                                break
            ...
            if time.time() - gd_start_time > MAX_DURATION:
                if last_audio:
                    print("⌛ Time up but user audio pending. Letting transcription finish...")
                    final_text = transcribe_audio(last_audio)
                    conversation.append(f"{username}: {final_text}")
